...
class SimplerAdapter(BaseEnvAdapter):
    def __init__(
        ...
        # Only True when running extract_hidden_states_and_actions.py (latent extraction
        # from Bridge TFRecords needs the raw, non-top-down-converted proprio frame).
...
class BridgeSimplerAdapter(SimplerAdapter):
    ...
    def preprocess_proprio(self, obs: dict) -> np.array:
        ...
        if self.collect_latents:
            rpy_bridge_converted = mat2euler(rm_bridge)         # ONLY FOR LATENT EXTRACTION
        else:
